...
against the SOURCE'S OWN native progression structure (Welsh CfW
Progression Steps 1-5, US/Ontario single grade, Scottish CfE Levels,
etc.). Single-band sources produce one indicator entry per LT at the
source's grade level; multi-band sources produce one entry per band
label in developmental order.

Self-reflection prompts come from the source's own
``ProgressionStructure.band_self_reflection_prompts`` map (calibrated
per source's own per-band developmental expectations) and are
inserted by the pipeline, not generated by the LLM.
...
def _validate_run(
    ...
    *,
    ...
    if not isinstance(raw_bands, list) or len(raw_bands) != progression.band_count:
        return None
    ...
    for i, expected_label in enumerate(expected_labels):
        ...
        clean_bands.append({"band": expected_label, "observable_behaviours": norm_behaviours})
    ...
    for b in majority_run["bands"]:
        bands.append(
            ObservationBand(
                ...
                self_reflection_prompt=progression.band_self_reflection_prompts.get(
                    b["band"], ""
                ),
            )
        )
...
def generate_observation_indicators_sync(
    ...
    **kwargs: Any,
    ...
    return asyncio.run(generate_observation_indicators(lt_set, progression, **kwargs))
